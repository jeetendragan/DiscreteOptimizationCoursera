...
public class Solver {
    ...
    public static void solve(String[] args) throws IOException {
        ...
        //System.out.println(bestState.toString(0,""));
...
class KnapsackSolver{
	...
	private static void UseNext(int itemIndex, String spaces, List<Item> items,
		...
		if(currentState.optimisticEvaluation < bestState.valueAttained){
			//System.out.println("currentState.optimisticEvaluation <= bestState.valueAttained");
			return; // Pruning the tree 
		}
		
		// check base case - i.e. have all the items been considered or is the
		// knapsack full
		...
		if(itemIndex == items.size()){
			...
			// Store the current state if it is greater than the best state seen till now
...
class ProblemState{
	...
	private void CalculateOptimisticEvaluation() {
		...
		optimisticEvaluation = opEvaluation;
	}
	...
	public ProblemState copyStateTo(ProblemState state){
		...
		for(int i = 0 ; i < taken.length; i++){
			state.taken[i] = taken[i];
		}
		
		state.currentWeight = currentWeight;
		state.valueAttained = valueAttained;
		state.optimisticEvaluation = optimisticEvaluation;
